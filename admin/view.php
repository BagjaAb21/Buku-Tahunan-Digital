    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
        ...
        <form class="form-horizontal" role="search" method="GET" action="">
          ...
          <div class="col-md-2">
            <button class="btn btn-default btn-block" type="submit">Cari</button>
          </div>
        </form>
        <br>
        <br>
...
<?php
    if (isset($_GET['cari'])){
      $q=mysql_query("SELECT * FROM murid INNER JOIN kelas ON murid.kode_kelas = kelas.id_kls WHERE murid.nama LIKE '%$_GET[cari]%' ORDER BY kelas.id_kls, murid.angkatan ASC");  
    }else{
      $q=mysql_query("SELECT * FROM murid INNER JOIN kelas ON murid.kode_kelas = kelas.id_kls ORDER BY murid.angkatan, kelas.id_kls ASC");
    }
    while($data=mysql_fetch_object($q))
  {
    ...
    <tbody>
      <tr>
        ...
        <td class="btn-group"><a href="hapus.php?id=<?php echo $data->id; ?>" class="btn btn-xs btn-danger" onclick="return confirm('Yakin data ini akan dihapus?');">Hapus</a>  <a href="view_detail.php?id=<?php echo $data->id; ?>" class="btn btn-xs btn-primary">Detail</a></td>
      </tr>
    </tbody>
    <?php 
  }
